...
SA_AEAC_CRS  <- crs(raster("F:\\Projects\\shrub_cover_paper\\Landsat_SA_CRS\\Landsat_SA_AEA.tif"))
...
Landsat.stack1  <- stack(sapply(X = Landsat.list[1:6],FUN = raster))
Landsat.stack1  <- crop(x = Landsat.stack1,y = study_area)
...
Landsat.stack2  <- stack(sapply(X = Landsat.list[7:12],FUN = raster))
Landsat.stack2  <- crop(x = Landsat.stack2,y = study_area)
...
Landsat  <- stack(Landsat.stack1,msavi.wet,ndvi.wet,Landsat.stack2,msavi.dry,ndvi.dry)
...
rm(Landsat.list,Landsat.stack1,Landsat.stack2)
rm(msavi.dry,msavi.wet,ndvi.dry,ndvi.wet)
